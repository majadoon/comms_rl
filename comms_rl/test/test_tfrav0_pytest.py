...
@pytest.fixture
def env():
    env = gym.make('CommsRLTimeFreqResourceAllocation-v0')  # Init environment
    yield env


class TestTfraV0:
    def test_reproducibility(self, env):
        env = gym.make('CommsRLTimeFreqResourceAllocation-v0', n_ues=64)  # Init environment
        env.seed(seed=1234)
    ...
    def test_state_features(self, env):
        n_ues = 64
        n_steps = 512
        env = gym.make('CommsRLTimeFreqResourceAllocation-v0', n_ues=n_ues, eirp_dbm=7)  # Low power to have some CQI=0
        env.seed(seed=1234)
